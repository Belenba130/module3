...
public class Repository<T, K> implements IRepository<T, K> {
    ...
    public T edit(T entity) {
        ...
        String fieldsUpdate = Arrays.stream(fields).filter(f -> f.getAnnotation(Id.class) == null).map(f -> f.getAnnotation(Column.class).name() + " = ?").collect(Collectors.joining(","));
        String key = Arrays.stream(fields).filter(f -> f.getAnnotation(Id.class) != null).map(f -> f.getAnnotation(Column.class).name()).collect(Collectors.joining(" "));

        String sql = MessageFormat.format("UPDATE {0} SET {1} WHERE {2} = ?", tblName, fieldsUpdate, key);

        return null;
    }

    @Override
    public boolean remove(K key, Class<T> entityClass) {
        Connection conn = null;
        PreparedStatement pst = null;
        try {
            conn = MySqlConnect.open();
